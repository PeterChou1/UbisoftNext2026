...
struct SIMDTriangle
{
    ...
    SIMDFloat EdgeFunc0(SIMDVec2& p) const { return B0 * (p.X - V1.X) - C0 * (p.Y - V1.Y); }

    SIMDFloat EdgeFunc1(SIMDVec2& p) const { return B1 * (p.X - V2.X) - C1 * (p.Y - V2.Y); }

    SIMDFloat EdgeFunc2(SIMDVec2& p) const { return B2 * (p.X - V3.X) - C2 * (p.Y - V3.Y); }

    void ComputeBarycentric(
            SIMDFloat& x, SIMDFloat& y, SIMDFloat& alpha, SIMDFloat& beta, SIMDFloat& gamma) const
    {

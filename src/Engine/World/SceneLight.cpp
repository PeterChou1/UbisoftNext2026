...
namespace SceneLighting
{
    namespace
    {
        ...
        // Axis aligned box around the scene's geometry (the renderer's vertex
        // buffer), false when there is none
        bool SceneBounds(Vec3& min, Vec3& max)
        {
            ...
            {
                min = Vec3(std::min(min.X, v.Position.X),
                           std::min(min.Y, v.Position.Y),
                           std::min(min.Z, v.Position.Z));
                max = Vec3(std::max(max.X, v.Position.X),
                           std::max(max.Y, v.Position.Y),
                           std::max(max.Z, v.Position.Z));
            }
            return true;
        }

        // Place and colour the renderer's light. A directional light's shadow
        // box is fitted around sceneMin .. sceneMax
        void Apply(Lighting& lighting,
                   ...
                   const Vec3& sceneMax)
        {
            ...
            if (ECS.HasResource<DepthBuffer>())
                texels = static_cast<float>(
                        std::min(ECS.GetResource<DepthBuffer>()->ShadowTexelsX(),
            ...
            if (light.Type == SceneLightType::Directional)
            {
                ...
                float left = 1e30f, right = -1e30f, bottom = 1e30f, top = -1e30f, nearest = 1e30f,
                      farthest = -1e30f;

...
{
  public:
    SIMDPixel(const SIMDVec2& position,
              const SIMDFloat& depth,
              const SIMDFloat& alpha,
              const SIMDFloat& beta,
    ...
    void Interpolate(Triangle& t)
    {
        ...
        auto inv1W = SIMDFloat(t.verts[0].InverseW);
        auto inv2W = SIMDFloat(t.verts[1].InverseW);
        auto inv3W = SIMDFloat(t.verts[2].InverseW);
        TextureCoord = (t1 * Alpha + t2 * Beta + t3 * Gamma) / Depth;
        Normal = (n1 * Alpha + n2 * Beta + n3 * Gamma) / Depth;
        WorldSpacePosition = (p1 * inv1W * Alpha + p2 * inv2W * Beta + p3 * inv3W * Gamma) / Depth;

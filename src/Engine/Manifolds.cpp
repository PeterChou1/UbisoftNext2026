...
CollisionCallback CollisionJumpTable[2][2] = {{Circle2Circle, Polygon2Circle},
                                              {Circle2Polygon, Polygon2Polygon}};

Manifold::Manifold(Entity idA, Entity idB, RigidBody& A, RigidBody& B)
    : idA(idA)
    , idB(idB)
    , A(A)
    , B(B)
...
void Manifold::ResolveCollisionAngular()
{
    ...
    for (int i = 0; i < ContactPoints.size(); i++)
    {
        ...
        float denom = A.InvMass() + B.InvMass() + (normalA * normalA) * A.InvInertia() +
    ...
    for (int i = 0; i < ContactPoints.size(); i++)
    {
        ...
        float denom = A.InvMass() + B.InvMass() + (normalAT * normalAT) * A.InvInertia() +
